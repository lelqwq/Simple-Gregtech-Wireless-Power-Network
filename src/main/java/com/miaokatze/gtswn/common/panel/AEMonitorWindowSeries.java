...
/**
 * AE 监视单窗口 61 点 FIFO 数据集。
 * <p>
 * 用于 AE 走势图/实时监控的 5m/1h/8h/24h 四个时间窗口，每个窗口固定保留 61 个采样点。
 * 超出容量时丢弃最旧的点（FIFO 滚动）。
 */
public class AEMonitorWindowSeries {

    /** 固定 61 个采样点（覆盖 5m / 1h / 8h / 24h 四档窗口） */
    public static final int CAPACITY = 61;

    // 内部存储：index 0 为最旧，size-1 为最新
    private final List<AEMonitorSample> data;

    public AEMonitorWindowSeries() {
     ...
     */
    public void add(AEMonitorSample sample) {
        if (sample == null) {
            return;
        }
     ...
     */
    public List<AEMonitorSample> copy() {
     ...
     * <p>
     * 用于"均值录入"：1h/8h/24h 集触发时，取上一级窗口最近 N 个点计算 rate 均值。
     *
     * @param n 要取的点数
     * @return 从旧到新的 N 个点副本；n <= 0 返回空列表；n >= size 返回全部点副本（等价于 copy()）
     */
    public List<AEMonitorSample> getLastN(int n) {
        int size = data.size();
        if (n <= 0) {
    ...
    public void readFromNBT(NBTTagCompound tag) {
        ...
        for (int i = start; i < list.tagCount(); i++) {
            AEMonitorSample s = AEMonitorSample.fromNBT(list.getCompoundTagAt(i));
            if (s != null) {
                data.add(s);
            }
        }
